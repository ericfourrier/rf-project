...
class GoogleFinanceStock(object):
    ...
    def get_data(self, convert_to_datetime=True):
        ...
        # split into lines
        res = requests.get(self.root_url, params=payload).text.strip().split('\n')
        ...
        self._data.loc[index_day, 'DATE_D'] = self._data.loc[
            index_day, 'DATE'].str.replace('a', '').map(lambda x: int(x))
        ...
        if convert_to_datetime:
            ...
            self._data['DATE'] = self._data['DATE'].map(
